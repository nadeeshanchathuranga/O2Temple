...
interface EditMembershipPackageModalProps {
    open: boolean;
...
const EditMembershipPackageModal: React.FC<EditMembershipPackageModalProps> = ({ 
    open, 
    onOpenChange, 
    package: pkg,
    availablePackages 
    ...
    useEffect(() => {
        if (pkg) {
            // Format birthday for date input (expects YYYY-MM-DD)
            let formattedBirthday = '';
            if (pkg.birthday) {
                const date = new Date(pkg.birthday);
                if (!isNaN(date.getTime())) {
                    formattedBirthday = date.toISOString().split('T')[0];
                }
            }

            // Calculate full payment based on package and sessions
            let calculatedFullPayment = pkg.full_payment;
            if (pkg.package_id) {
                const selectedPackage = availablePackages.find(p => p.id === pkg.package_id);
                if (selectedPackage) {
                    calculatedFullPayment = selectedPackage.price * pkg.num_of_sessions;
                }
            }
            ...
            setData({
                ...
                nic: pkg.nic || '',
                phone: pkg.phone,
                num_of_sessions: pkg.num_of_sessions,
                discount_percentage: pkg.discount_percentage,
                full_payment: calculatedFullPayment,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (pkg) {
            put(`/membership-packages/${pkg.id}`, {
                onSuccess: () => {
                    onOpenChange(false);
                    reset();
                },
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto p-0 gap-0 bg-white">
                <div className="relative bg-white rounded-lg">
                    <div className="flex items-center gap-3 p-6 pb-4 border-b border-gray-100 sticky top-0 bg-white z-10">
                        ...
                        <h2 className="text-lg font-semibold text-gray-900">Edit Membership Package</h2>
                        <button
                            ...
                            <div>
                                ...
                                {pkg?.package && (
                                    <p className="text-xs text-blue-600 mt-1">
                                        Current package: {pkg.package.name} - {pkg.package.duration_minutes} min - Rs {Number(pkg.package.price).toFixed(2)}
                                    </p>
                                )}
                            </div>
                            ...
                            {pkg && (
                                <div className="p-4 bg-blue-50 rounded-lg">
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="text-sm font-medium text-blue-700">Sessions Used:</span>
                                        <span className="text-lg font-bold text-blue-900">
                                            {pkg.sessions_used} / {pkg.num_of_sessions}
                                        </span>
                                    </div>
                                    <div className="w-full bg-blue-200 rounded-full h-2">
                                        <div 
                                            className="bg-blue-600 h-2 rounded-full transition-all"
                                            style={{ width: `${(pkg.sessions_used / pkg.num_of_sessions) * 100}%` }}
                                        ></div>
                                    </div>
                                </div>
                            )}
